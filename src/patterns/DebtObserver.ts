...
export class DebtObserver implements TransactionObserver {
  async onTransactionCreated(transaction: Transaction, context?: TransactionContext): Promise<Model[]> {
  ...
  async onTransactionDeleted(transaction: Transaction, context?: TransactionContext): Promise<Model[]> {
    if (!context?.debtId) return []
